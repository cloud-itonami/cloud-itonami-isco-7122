(ns floortile.governor
  "FloorTileGovernor — the independent safety/scope layer gating every
  job-site scheduling/logistics proposal an advisor may make for a
  floor-laying/tile-setting crew. The governor never dispatches
  hardware itself, never performs flooring/tile-installation work on
  the job site, and never finalizes a flooring/tile-installation-
  execution decision (e.g. deciding to proceed with a specific
  flooring or tile installation step) or overrides a site safety
  ...
  SCHEDULING/LOGISTICS ONLY — it never performs flooring/tile-
  installation work itself). Modeled on cloud-itonami-isco-7111's
  housebuilder.governor (job-site scheduling/logistics coordination
  shape) and cloud-itonami-isco-3313's accountingsupport.governor.

  HARD invariants (:hard? true, ALWAYS :hold, never overridable):
    1. installer provenance  — the crew member must be independently
    ...
    3. no-actuation           — proposal :effect must be :propose (the
                                ...
                                never performs flooring/tile-
                                installation work itself; it only
    ...
    5. scope-excluded action  — any proposal to directly finalize a
                                flooring/tile-installation-execution
                                decision (e.g. deciding to proceed
                                with a specific flooring or tile
                                installation step), or to override a
                                site safety officer's judgment, is a
                                ...
                                tile installation\" / \"authorize the
                                flooring installation\" / \"override
                                the site safety officer's judgment\",
                                never as bare nouns like \"tile\",
                                \"flooring\" or \"safety\", so the
                                check can never self-trip on the
                                advisor's own routine rationale text,
                                e.g. \"logged work record for
                                installer …\" or \"scheduled crew
                                operation for tile-setting task …\" or
  ...
  regardless of confidence):
    6. :op :flag-safety-concern (a fume-exposure/cut-hazard/site-
...
;; ("tile", "flooring", "safety", "officer") — so this can never
;; match inside the mock advisor's own default rationale text (which
;; legitimately contains those bare nouns, e.g. "tile-setting task" /
;; "site safety officer review"). See
;; `governor-test/default-mock-advisor-proposals-never-self-trip-on-scope-exclusion`.
(def ^:private scope-excluded-phrases
  ["proceed with the tile installation" "proceed with the flooring installation"
   "finalize the tile installation decision" "finalize the flooring installation decision"
   "authorize the tile installation" "authorize the flooring installation"
  ...
  (let [{:keys [op rationale]} proposal]
    (cond-> []
      ...
      (conj {:rule :no-installer
             :detail "未登録 installer への提案は不可（installer record は独立して検証・登録済みでなければならない）"})
      ...
      (conj {:rule :scope-excluded-action
             :detail "フローリング/タイル施工の実行判断の確定・site safety officer の判断の上書きは、この actor の権限外 — 常に永続ブロック"}))))

(defn check
  "Assess a proposal against `request`/`context`/`proposal` and a
  `store` implementing `floortile.store/Store`. Pure — never mutates
